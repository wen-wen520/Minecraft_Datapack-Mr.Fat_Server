...
execute store result score day w_w.daytime store result storage w_w:datapack/mr_fat/utilities/auto_sleep day int 1 run time query day
# message
tellraw @a ["§b服务器 §7> §f快到 §a第",{"score": {"name": "day","objective": "w_w.daytime"},"color":"green"},"§a天 §f的晚安时间了, 即将自动跳过这个夜晚"]
